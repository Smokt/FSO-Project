...
sem_t mutex_SV;
sem_t mutex_CL;
sem_t hay_dato_R[NUM_HCALC];
sem_t hay_hueco_R[NUM_HCALC];
...
int main()
{
  ...
  for(i=0;i<NUM_HCALC;i++){
    sem_init(&hay_dato_R[i],0,0);
  }
  for(i=0;<NUM_HCALC;i++){
    sem_init(&hay_hueco_R[i],0,1);
  }
...
void calcula_i(int *numHil) //necesito coger el valor de numHil
{
  while(1) {
    ...
    celdaLibre=*numHil;
    sem_post(&mutex_CL);
    sem_wait(&hay_hueco_R[*numHil]);
    R[*numHil].fila=filaRegistro_i;
    R[*numHil].D=resultado;
    sem_post(&hay_dato_R[*numHil]);
...
void recoge ( ) {
  ...
  while(1){
    ...
    celda = celdaLibre;
    sem_post(&mutex_CL);
    sem_wait(&hay_dato_R[celda]);
    aux=R[celda].D;
    auxLinea=R[celda].fila;
    sem_post(&hay_hueco_R[celda]);
